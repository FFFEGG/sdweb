...
import { Modal, Select as Selects } from "@douyinfe/semi-ui";

const UserBottleEntry = ({ customization }) => {
    ...
    const [units, setunits] = useState(initData.CompanyInfo.appversion.units)
    const { register, handleSubmit, control, reset } = useForm({
        defaultValues: {
            province: moment(new Date()).format('YYYY-MM-DD'),
            ...
            production_number: '',
            volume: '35.5',
            nominal_pressure: '2.1',
            wall_thickness: '2.5',
            weight: '16',
            remarks: loginuser.name + '办理用户带瓶入户'
    ...
    const onSubmit = async (data) => {
        ...
        //验证code格式 识别码SR00000000 SR开头10位数字
        if (!/^SR\d{8}$/.test(code)) {
            toast.error('识别码格式错误,SR开头10位数字')
            return
        }

        Modal.confirm({
            title: '提示',
            content: '确认办理用户带瓶入户资料？',
            style: {
                top: '30%'
            },
            onOk: async () => {
                const rew = await request('post', '/api/getInfo', {
                    ...data,
                    url: 'Srapp.Web_BusinessProcessing_Handle.CollectUserPackingtypeMaterial',
                ...
                if (rew.data.msg === 'SUCCESS') {
                    ...
                    toast.success('录入失败' + rew.data.tips)
                }
            }
        })
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <Box marginBottom={3} p={3} bgcolor="#fff" borderRadius={1}>
                <TextField {...register("code")} label="识别码SR00000000" sx={{ marginBottom: 2, width: 200 }} InputLabelProps={{ shrink: true }} />
                {/* <TextField {...register("property_unit")} label="产权单位" sx={{ marginBottom: 2, width: 200 }} InputLabelProps={{ shrink: true }} /> */}
                ...
                <FormControl sx={{ minWidth: 200, marginBottom: 2 }}>
                    <Controller
                        ...
                        render={({ field }) =>
                            <>
                                <InputLabel shrink="true">产权单位</InputLabel>
                                <Selects size="large" style={{ width: 200, height: 51.3, border: '1px solid #ccc' }} filter  {...field} label="产权单位" inputProps={{ shrink: true }}>
                                    {
                                        units.map(item => <Selects.Option value={item}>{item}</Selects.Option>)
                                    }
                                </Selects>
                            </>
                ...
                <FormControl sx={{ minWidth: 200, marginBottom: 2 }}>
                    <Controller
                        ...
                        render={({ field }) =>
                            <>
                                <InputLabel shrink="true">制造单位</InputLabel>
                                <Selects size="large" style={{ width: 200, height: 51.3, border: '1px solid #ccc' }} filter  {...field} label="制造单位" inputProps={{ shrink: true }}>
                                    {
                                        factory.map(item => <Selects.Option value={item.label}>{item.label}</Selects.Option>)
                                    }
                                </Selects>
                            </>
                ...
                <FormControl sx={{ width: 200, marginBottom: 2 }} >
                    <Controller
                        ...
                        render={({ field }) =>
                            <>
                                <InputLabel shrink="true">包装物类型</InputLabel>
                                <Select   {...field} label="包装物类型" inputProps={{ shrink: true }}>
                                    {
                ...
                <TextField {...register('reg_number')} label="登记编号（钢印）" sx={{ marginBottom: 2, width: 200 }}
                    InputLabelProps={{ shrink: true }} />
                <TextField {...register('production_number')} label="出厂编号" sx={{ marginBottom: 2, width: 200 }}
                    InputLabelProps={{ shrink: true }} />
                <TextField {...register('date4manufacture')} label="生产日期" type="date" sx={{ marginBottom: 2, width: 200 }}
                    InputLabelProps={{ shrink: true }} />
                <TextField {...register('lasttestdate')} label="最后检测日期" sx={{ marginBottom: 2, width: 200 }} type="date"
                    InputLabelProps={{ shrink: true }} />
                <TextField {...register('nexttestdate')} label="下此检测日期" sx={{ marginBottom: 2, width: 200 }} type="date"
                    InputLabelProps={{ shrink: true }} />
                <TextField {...register('volume')} label="容积（L）" sx={{ marginBottom: 2, width: 200 }}
                    InputLabelProps={{ shrink: true }} />
                <TextField {...register('wall_thickness')} label="设计壁厚（MM）" sx={{ marginBottom: 2, width: 200 }}
                    InputLabelProps={{ shrink: true }} />
                <TextField {...register('nominal_pressure')} label="公称压力(Mpa)" sx={{ marginBottom: 2, width: 200 }}
                    InputLabelProps={{ shrink: true }} />
                <TextField {...register('weight')} label="瓶重KG" sx={{ marginBottom: 2, width: 200 }}
                    InputLabelProps={{ shrink: true }} />
                <TextField {...register('remarks')} label="备注" sx={{ marginBottom: 2, width: 200 }} InputLabelProps={{ shrink: true }} />
                <Box marginTop={2}>
                    <Button type="submit" variant="outlined" size="medium">确认提交</Button>
                </Box>
            </Box>
        </form >
    );
...
export default connect(mapStateToProps)(UserBottleEntry);
