...
const BuyUserPackingtypeMaterial = () => {
    const factory = [
        {
            value: 1,
            label: "安徽省枞阳县压力容器厂"
    ...
    const [zd_price, setZd_price] = useState(0)
    const onsubmit = async (data) => {
        ...
        //收购价不能大于指导价
        if (data.price > zd_price) {
            toast.error('收购价不能大于指导价')
            return
        }
        ...
        Modal.confirm({
            ...
            onOk: async () => {
                const rew = await request('post', '/api/getInfo', {
                    ...data,
                    marketprice: zd_price,
                    url: 'Srapp.Web_BusinessProcessing_Handle.BuyUserPackingtypeMaterial',
                    remarks: data.is_bf == '是' ? data.remarks + ',报废瓶' : data.remarks
                })
    ...
    const getGuidePrice = (packingtype, date4manufacture, lasttestdate, nature, is_bf) => {
        const arr = initData.BuyPackingtypeParameterList[0].detailed
        if (is_bf) {
            // if (packingtype === 'YSP12型钢瓶') {
            //     return 5
            // }
            // if (packingtype === 'YSP35.5型钢瓶' || packingtype === 'YSP28.6型钢瓶') {
            //     return 20
            // }
            //
            // if (packingtype === 'YSP118型钢瓶') {
            //     return 50
            // }

            return Math.min(...arr.filter(item=>item.packagingType == packingtype && item.isOwnUnit == nature).map(item => item.price));

        }

        const data = arr.filter(e => e.packagingType === packingtype && e.productionYear === date4manufacture && e.inspectionYear === lasttestdate && e.isOwnUnit === nature)
    ...
    return (
        <Box sx={{ width: '100%', background: '#FFF' }}>
            <Form getFormApi={e => api.current = e} onSubmit={e => onsubmit(e)} onChange={e => {
                console.log(e)
                const data = e.values

                const price = getGuidePrice(data.packingtype, (data.date4manufacture?.substr(0, 4) ?? 0) * 1, (data.lasttestdate?.substr(0, 4) ?? 0) * 1, data.nature == '收购(本单位)', data.is_bf == '是')
                // console.log(price)
                // api.current.setValue('zd_price', price)
                setZd_price(price)
            }}>
                <Box p={3} bgcolor="#fff" borderRadius={1} overflow="scroll" display="flex">
                    <Box border={1} p={2} borderColor="#999">
                        <Typography fontSize={23} textAlign="center" fontWeight="bold"
                            marginBottom={2}>收购包装物物资业务</Typography>
                        ...
                        <table className="BuyUserPackingtypeMaterialtable">
                            <thead>
                                <tr>
                                    ...
                                    <td>
                                        <Form.Input placeholder={'SR开头10位数字'} noLabel={true} field='code' type="text" />
                                    </td>
                                    <th>
                                        产权单位
                                    </th>
                                    <td>
                                        ...
                                        <Form.Select noLabel={true} size="large" field='property_unit' initValue={'三燃'} filter>
                                            {
                                                units.map(item => <Form.Select.Option value={item}>{item}</Form.Select.Option>)
                                            }
                                        </Form.Select>
                                    </td>
                                    <th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <Form.Input style={{ minWidth: 150 }} initValue={moment().format('YYYY')} noLabel={true} field='date4manufacture' type="number" />
                                    </td>
                                    <th>
                                        最近检测年限
                                    </th>
                                    <td>
                                        ...
                                        <Form.Input initValue={moment().format('YYYY')} noLabel={true} field='lasttestdate' type="number" />
                                    </td>
                                    <th>
                                        下次检测年限
                                    </th>
                                    <td>
                                        <Form.Input initValue={moment().format('YYYY')} noLabel={true} field='nexttestdate' type="number" />
                                    </td>
                                </tr>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <Form.Input noLabel={true} initValue={35.5} field='volume' type="text" />
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        <Form.Input noLabel={true} initValue={2.1} field='wall_thickness' type="text" />
                                    </td>
                                    <th>
                                        公称压力(Mpa)
                                    ...
                                    <td>
                                        ...
                                        <Form.Input noLabel={true} initValue={2.5} field='nominal_pressure' type="text" />
                                    </td>
                                </tr>
                                <tr>
                                    <th>
                                    ...
                                    <td>
                                        ...
                                        <Form.Input noLabel={true} initValue={12} field='weight' type="text" />
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        {/* <Form.Input noLabel={true} field='zd_price' type="text" /> */}
                                    </td>
                                </tr>
                                <tr>
                                    <th>
                ...
                <Box px={3} pb={3}>
                    <Button sx={{ fontSize: 20 }} type="submit" variant="contained">确认办理</Button>
                </Box>
            </Form>
            {/*<Box p={3}>*/}
            {/*    <CylinderPriceTable />*/}
...
export default BuyUserPackingtypeMaterial;
