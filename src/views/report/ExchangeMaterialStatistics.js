...
const ExchangeMaterialStatistics = () => {
  const [list, setList] = useState([]);

  return (
    <Box p={3} bgcolor={"#fff"} borderRadius={1}>
      <Box fontSize={18} mb={3}>
        获取部门交换物资统计(员工交接/员工交接(签封))
      </Box>

      <Form
        onSubmit={async (e) => {
          const rew = await request("post", "/api/getInfo", {
            url: "Srapp.Web_Report_Material_Infos.ExchangeMaterialStatistics",
            ...e,
            issuedepartment: JSON.stringify(e.issuedepartment),
            receivedepartment: JSON.stringify(e.receivedepartment),
...
export default ExchangeMaterialStatistics;
