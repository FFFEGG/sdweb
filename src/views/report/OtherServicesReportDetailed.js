...
const OtherServicesReportDetailed = () => {
    const initData = JSON.parse(localStorage.getItem('initData'))
    const loginuser = JSON.parse(localStorage.getItem('userinfo'))
    const [list,setList] = useState([])
    return (
        <Box p={3} bgcolor="#fff">
            <Typography fontSize={20} mb={3}>其它服务报表明细</Typography>
            <Form onSubmit={async e=>{
                const rew = await request('post','/api/getInfo',{
                    url: 'Srapp.Web_Report_Business_Infos.OtherServicesReportDetailed',
                    ...e,
                ...
                <Form.Select field='servicepersonal'  maxTagCount={1} multiple label="服务人员" style={{ width: 200}}>
                    {
                ...
                <Form.Select field='servicetype'  maxTagCount={1} multiple label="服务类型" style={{ width: 200}}>
                    <Form.Select.Option value="维修钢瓶">维修钢瓶</Form.Select.Option>
                    <Form.Select.Option value="维修热水器">维修热水器</Form.Select.Option>
                </Form.Select>
...
export default OtherServicesReportDetailed;
