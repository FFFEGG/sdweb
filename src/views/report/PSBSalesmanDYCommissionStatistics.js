...
const PSBSalesmanDYCommissionStatistics = () => {
     ...
     return (
          <Box p={3} bgcolor={'white'} borderRadius={1}>
               <Box fontSize={18} mb={3}>配送部 业务员当月开户提成</Box>
               <Form getFormApi={e => api.current = e} layout="horizontal" labelPosition="inset" onSubmit={async e => {
                    const rew = await request('post', '/api/getInfo', {
                         url: 'Srapp.Web_Report_Business_Infos.PSBSalesmanDYCommissionStatistics',
                         ...e,
                    ...
                    if (rew.data.info.length) {
                         rew.data.info.push({
                              ...
                              "commission": (rew.data.info.reduce((a, b) => a + parseFloat(b.commission), 0)),
                         })
                    ...
                    <Form.Select maxTagCount={2} multiple filter field="salesman" label="业务员">
                         {
               ...
               <Box mt={3} height={'60vh'}>
                    <AgGridReact
                         ...
                         columnDefs={[
                              ...
                              { field: 'salesman', headerName: '业务员', sortable: true, filter: true, },
                              ...
                              { field: 'benchmarkprice', headerName: '基准价', sortable: true, filter: true, },
                              { field: 'num', headerName: '数量', sortable: true, filter: true, },
                              { field: 'commission', headerName: '提成', sortable: true, filter: true, valueGetter: ({ data }) => (data?.commission).toFixed(2) },
...
export default PSBSalesmanDYCommissionStatistics;