     ...
     return (
          <Box p={3} bgcolor={'white'} borderRadius={1}>
               ...
               <Form getFormApi={e => api.current = e} layout="horizontal" labelPosition="inset" onSubmit={async e => {
                    ...
                    // })
                    if (rew.data?.info?.length > 0) {
                         rew.data?.info?.map(item => {
                              item.lsxl = parseFloat(item.lsjtsales) + parseFloat(item.lssysales) - parseFloat(item.lsresidual)
                              item.syqxl = parseFloat(item.syqsales) - parseFloat(item.syqKG45residual) - parseFloat(item.syqotherresidual)
                              item.total = parseFloat(item.lsxl) + parseFloat(item.syqxl)
                         })
                         rew.data.info.push({
                              department: '合计',
                              lsjtkh: rew.data.info.reduce((a, b) => a + b.lsjtkh, 0),
                              lssykh: rew.data.info.reduce((a, b) => a + b.lssykh, 0),
                              lsjtsales: rew.data.info.reduce((a, b) => a + b.lsjtsales, 0),
                              lssysales: rew.data.info.reduce((a, b) => a + b.lssysales, 0),
                              lsresidual: rew.data.info.reduce((a, b) => a + b.lsresidual, 0),
                              syqsales: rew.data.info.reduce((a, b) => a + b.syqsales, 0),
                              syqKG45residual: rew.data.info.reduce((a, b) => a + b.syqKG45residual, 0),
                              syqotherresidual: rew.data.info.reduce((a, b) => a + b.syqotherresidual, 0),
                              lssales: rew.data.info.reduce((a, b) => a + b.lssales, 0),
                              lsxl: rew.data.info.reduce((a, b) => a + b.lsxl, 0),
                              syqxl: rew.data.info.reduce((a, b) => a + b.syqxl, 0),
                              total: rew.data.info.reduce((a, b) => a + b.total, 0),
                         })
               ...
               <Box mt={3} height={'60vh'}>
                    <AgGridReact
                         ...
                         columnDefs={[
                              ...
                              //      "lssales": 0
                              //  }
                              { field: 'department', headerName: '商店', sortable: true, filter: true, width: 100, },
                              {
                                   headerName: '开户数', children: [
                                        { field: 'lsjtkh', headerName: '零售家庭', sortable: true, filter: true, width: 100, },
                                        { field: 'lssykh', headerName: '零售商业', sortable: true, filter: true, width: 100, },
                                   ]
                              },

                              {
                                   headerName: '销量', children: [
                                        { field: 'lsjtsales', headerName: '零售家庭', sortable: true, filter: true, width: 100, },
                                        { field: 'lssysales', headerName: '零售商业', sortable: true, filter: true, width: 100, },
                                        { field: 'syqsales', headerName: '商业气', sortable: true, filter: true, width: 100, },

                                   ]
                              },
                              {
                                   headerName: '残液', children: [
                                        { field: 'syqKG45residual', headerName: '45KG', sortable: true, filter: true, width: 100, },
                                        { field: 'syqotherresidual', headerName: '其他规格', sortable: true, filter: true, width: 100, },
                                        { field: 'lsresidual', headerName: '零售', sortable: true, filter: true, width: 100, },
                                   ]
                              },

                              {
                                   headerName: '实际销量', children: [
                                        { field: 'lsxl', headerName: '零售销量', sortable: true, filter: true, width: 100, },
                                        { field: 'syqxl', headerName: '商业气销量', sortable: true, filter: true, width: 100, },
                                        { field: 'total', headerName: '商店总销量', sortable: true, filter: true, width: 100, },
...
export default LSHQUserSalesStatistics;