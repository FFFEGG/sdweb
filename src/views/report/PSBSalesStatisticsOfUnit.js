...
const PSBSalesStatisticsOfUnit = () => {
     ...
     return (
          <Box p={3} bgcolor={'white'} borderRadius={1}>
               <Box fontSize={18} mb={3}>配送部 销售统计 按包装单位</Box>
               <Form getFormApi={e => api.current = e} layout="horizontal" labelPosition="inset" onSubmit={async e => {
                    const rew = await request('post', '/api/getInfo', {
                         url: 'Srapp.Web_Report_Business_Infos.PSBSalesStatisticsOfUnit',
                         ...e,
                    ...
                    if (rew.data.info.length) {
                         rew.data.info.push({
                              department: '合计',
                              bigbarrelnum: rew.data.info.reduce((a, b) => a + b.bigbarrelnum, 0),
                              smallbarrelnum: rew.data.info.reduce((a, b) => a + b.smallbarrelnum, 0),
                              waterdispensernum: rew.data.info.reduce((a, b) => a + b.waterdispensernum, 0),
                              supportingwaternum: rew.data.info.reduce((a, b) => a + b.supportingwaternum, 0),
                              supportingwatertotal: rew.data.info.reduce((a, b) => a + b.supportingwatertotal, 0),
                              waterpumptotal: rew.data.info.reduce((a, b) => a + b.waterpumptotal, 0),
                              oneusebarrelnum: rew.data.info.reduce((a, b) => a + b.oneusebarrelnum, 0),
                         })
                    ...
                    <Form.TreeSelect treeData={new_department_byname} leafOnly field="department" label="业务部门" maxTagCount={1} multiple />
               ...
               <Box mt={3} height={'60vh'}>
                    <AgGridReact
                         ...
                         columnDefs={[
                              ...
                              //      "supportingwatertotal": 0,
                              //      "waterpumptotal": 0,
                              //      "oneusebarrelnum": 0
                              // }
                              { field: 'department', headerName: '业务部门', sortable: true, filter: true, },
                              { field: 'bigbarrelnum', headerName: '大桶', sortable: true, filter: true, },
                              { field: 'smallbarrelnum', headerName: '小桶', sortable: true, filter: true, },
                              { field: 'waterdispensernum', headerName: '饮水机', sortable: true, filter: true, },
                              { field: 'supportingwaternum', headerName: '支装水', sortable: true, filter: true, },
                              { field: 'supportingwatertotal', headerName: '支装水(金额)', sortable: true, filter: true, },
                              { field: 'waterpumptotal', headerName: '无线抽水器(金额)', sortable: true, filter: true, },
                              { field: 'oneusebarrelnum', headerName: '一次性桶', sortable: true, filter: true, },
...
export default PSBSalesStatisticsOfUnit;