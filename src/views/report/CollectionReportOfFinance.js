...
const CollectionReportOfFinance = () => {
    ...
    const getCollectionReportDetail = async (data, type) => {
        ...
        rew.data.info.push({
            date: '合计',
            num: gettotal(rew.data.info, 'num'),
        })
    ...
    return (
        <Box p={3} bgcolor="#fff">
            ...
            <Form getFormApi={e => api.current = e} onChange={e => {
                ...
                const rew = await request('post', '/api/getInfo', {
                    url: 'Srapp.Web_Report_Finance_Infos.CollectionReportOfFinance',
                    ...e,
            ...
            <Box id="table1">
                ...
                <div style={{
                    ...
                    <div>
                        部门:
                        {api.current?.getValue('department')?.join(',')}
                    </div>
                </div>
                <h3>收款报表</h3>
                <table className={'my-table'} style={{ fontSize: 12, marginTop: 10 }}>
                    <thead>
                    <tr>
                        <td style={{ fontSize: 12 }} colSpan={12}>收款报表(收)</td>
                    </tr>
                    <tr>

                        {/*<th>方式</td>*/}
                        <td style={{ fontSize: 12 }}>项目</td>
                        {/*<th>商品</td>*/}
                        <td style={{ fontSize: 12 }}>收支</td>
                        <td style={{ fontSize: 12 }}>数量</td>
                        {/*<th>包装物</td>*/}
                        <td style={{ fontSize: 12 }}>现结欠款</td>
                        <td style={{ fontSize: 12 }}>月结欠款</td>
                        <td style={{ fontSize: 12 }}>余额支付</td>
                        <td style={{ fontSize: 12 }}>现金支付</td>
                        <td style={{ fontSize: 12 }}>专项款支付</td>
                        {/*<td style={{ fontSize: 12 }}>优惠券支付</td>*/}
                        <td style={{ fontSize: 12 }}>在线支付</td>
                        <td style={{ fontSize: 12 }}>库存款支付</td>
                        <td style={{ fontSize: 12 }}>合计</td>
                    </tr>
                    </thead>
                    <tbody>
                    {
                        getlist(list, '收').map(item => <tr style={{ color: item.inandout == '支' ? 'red' : 'black' }}>
                            ...
                            <td style={{ cursor: 'pointer' }} ><div onClick={() => getCollectionReportDetail(item, '现结欠款')}>{parseFloat(item.casharrears).toFixed(2)}</div></td>
                            <td style={{ cursor: 'pointer' }} ><div onClick={() => getCollectionReportDetail(item, '月结欠款')}>{item.payarrears.toFixed(2)}</div></td>
                            <td style={{ cursor: 'pointer' }} ><div onClick={() => getCollectionReportDetail(item, '余额支付')}>{item.paybalance.toFixed(2)}</div></td>
                            <td style={{ cursor: 'pointer' }} ><div onClick={() => getCollectionReportDetail(item, '现金支付')}>{item.paycash.toFixed(2)}</div></td>
                            <td style={{ cursor: 'pointer' }} ><div onClick={() => getCollectionReportDetail(item, '专项款支付')}>{item.paycashgift.toFixed(2)}</div></td>
                            {/*<td style={{ cursor: 'pointer' }} ><div onClick={() => getCollectionReportDetail(item, '优惠券支付')}>{item.paycoupon}</div></td>*/}
                            <td style={{ cursor: 'pointer' }} ><div onClick={() => getCollectionReportDetail(item, '在线支付')}>{item.payonline.toFixed(2)}</div></td>
                            <td style={{ cursor: 'pointer' }} ><div onClick={() => getCollectionReportDetail(item, '库存款支付')}>{item.paystock.toFixed(2)}</div></td>
                            <td style={{ cursor: 'pointer' }} ><div onClick={() => getCollectionReportDetail(item, '合计')}>{item.total.toFixed(2)}</div></td>
                        </tr>)
                    }
                ...
                <table className={'my-table'} style={{ fontSize: 12, marginTop: 10 }}>
                    <thead>
                    <tr>
                        <td style={{ fontSize: 12 }} colSpan={12}>收款报表(支)</td>
                    </tr>
                    <tr >
                    ...
                    <tbody>
                    {
                        getlist(list, '支').map(item => <tr style={{ color: item.inandout == '支' ? 'red' : 'black' }}>
                            ...
                            {/*<td>{item.goodsname}</td>*/}
                            <td>{item.inandout}</td>
                            <td>{parseFloat(item.num).toFixed(2)}</td>
                            {/*<td>{item.packingtype}</td>*/}
                            <td style={{ cursor: 'pointer' }} ><div onClick={() => getCollectionReportDetail(item, '现结欠款')}>{parseFloat(item.casharrears).toFixed(2)}</div></td>
                            <td style={{ cursor: 'pointer' }} ><div onClick={() => getCollectionReportDetail(item, '月结欠款')}>{parseFloat(item.payarrears).toFixed(2)}</div></td>
                            <td style={{ cursor: 'pointer' }} ><div onClick={() => getCollectionReportDetail(item, '余额支付')}>{parseFloat(item.paybalance).toFixed(2)}</div></td>
                            <td style={{ cursor: 'pointer' }} ><div onClick={() => getCollectionReportDetail(item, '现金支付')}>{parseFloat(item.paycash).toFixed(2)}</div></td>
                            <td style={{ cursor: 'pointer' }} ><div onClick={() => getCollectionReportDetail(item, '专项款支付')}>{parseFloat(item.paycashgift).toFixed(2)}</div></td>
                            {/*<td style={{ cursor: 'pointer' }} ><div onClick={() => getCollectionReportDetail(item, '优惠券支付')}>{item.paycoupon}</div></td>*/}
                            <td style={{ cursor: 'pointer' }} ><div onClick={() => getCollectionReportDetail(item, '在线支付')}>{parseFloat(item.payonline).toFixed(2)}</div></td>
                            <td style={{ cursor: 'pointer' }} ><div onClick={() => getCollectionReportDetail(item, '库存款支付')}>{parseFloat(item.paystock).toFixed(2)}</div></td>
                            <td style={{ cursor: 'pointer' }} ><div onClick={() => getCollectionReportDetail(item, '合计')}>{parseFloat(item.total).toFixed(2)}</div></td>
                        </tr>)
                    }
                    <tr style={{ color: 'black' }}>
                        <td>合计</td>
                        <td></td>
                        <td >{gettotal(getlist(list, '支'), 'num').toFixed(2)}</td>
                        <td >{gettotal(getlist(list, '支'), 'casharrears').toFixed(2)}</td>
                        <td >{gettotal(getlist(list, '支'), 'payarrears').toFixed(2)}</td>
                        <td >{gettotal(getlist(list, '支'), 'paybalance').toFixed(2)}</td>
                        <td >{gettotal(getlist(list, '支'), 'paycash').toFixed(2)}</td>
                        <td >{gettotal(getlist(list, '支'), 'paycashgift').toFixed(2)}</td>
                        {/*<td >{gettotal(getlist(list, '支'), 'paycoupon')}</td>*/}
                        <td >{gettotal(getlist(list, '支'), 'payonline').toFixed(2)}</td>
                        <td >{gettotal(getlist(list, '支'), 'paystock').toFixed(2)}</td>
                        <td >{gettotal(getlist(list, '支'), 'total').toFixed(2)}</td>
                    </tr>
                    </tbody>
                </table>
                ...
                <table className={'my-table'} style={{ fontSize: 12, marginTop: 10 }}>
                    <thead>
                    <tr>
                        <td style={{ fontSize: 12 }} colSpan={4}>附表（水票）</td>
                    </tr>
                    <tr>
                        <td style={{ fontSize: 12 }}>商品名称</td>
                        <td style={{ fontSize: 12 }}>单价</td>
                        <td style={{ fontSize: 12 }}>数量</td>
                        <td style={{ fontSize: 12 }}>小计</td>
                    </tr>
                    </thead>
                    <tbody>
                    {
                    ...
                    <tr>
                        ...
                        <td>{physicalcoupon.reduce((a, b) => a + parseFloat(b.price * b.num), 0).toFixed(2)}</td>
                ...
                <table className={'my-table'} style={{ fontSize: 12, marginTop: 10 }}>
                    <thead>
                    <tr>
                        <td style={{ fontSize: 12 }} colSpan={4}>附表（包装物收）</td>
                    </tr>
                    <tr>
                        <td style={{ fontSize: 12 }}>方式</td>
                        <td style={{ fontSize: 12 }}>商品名称</td>
                        <td style={{ fontSize: 12 }}>数量</td>
                        <td style={{ fontSize: 12 }}>小计</td>
                    </tr>
                    ...
                    <tbody>
                    {
                        getlist(list, '包装物')?.map(item => <tr>
                            ...
                            <td>{item.goodsname}</td>
                            <td>{item.num}</td>
                            <td>{parseFloat(item.paycash + item.paybalance).toFixed(2)}</td>
                        </tr>)
                    }
                    <tr>
                        <td style={{ fontSize: 12 }}>合计</td>
                        <td style={{ fontSize: 12 }}></td>
                        <td style={{ fontSize: 12 }}>{getlist(list, '包装物')?.reduce((a, b) => a + parseFloat(b.num), 0)}</td>
                        <td style={{ fontSize: 12 }}>{parseFloat(getlist(list, '包装物')?.reduce((a, b) => a + parseFloat(b.paycash), 0) + getlist(list, '包装物')?.reduce((a, b) => a + parseFloat(b.paybalance), 0)).toFixed(2)}</td>
                    </tr>
                    </tbody>
                ...
                <table className={'my-table'} style={{ fontSize: 12, marginTop: 10 }}>
                    <thead>
                    <tr>
                        <td style={{ fontSize: 12 }} colSpan={4}>附表（包装物支）</td>
                    </tr>
                    <tr>
                        <td style={{ fontSize: 12 }}>方式</td>
                        <td style={{ fontSize: 12 }}>商品名称</td>
                    ...
                    <tr>
                        <td style={{ fontSize: 12 }}>合计</td>
                        <td style={{ fontSize: 12 }}></td>
                        <td style={{ fontSize: 12 }}>{getlist(list, '退押金款')?.reduce((a, b) => a + parseFloat(b.num), 0)}</td>

                        <td style={{ fontSize: 12 }}>{parseFloat(getlist(list, '退押金款')?.reduce((a, b) => a + parseFloat(b.paycash), 0) + getlist(list, '包装物')?.reduce((a, b) => a + parseFloat(b.paybalance), 0)).toFixed(2)}</td>
                ...
                <table className={'my-table'} style={{ fontSize: 12, marginTop: 10 }}>
                    <thead>
                    <tr>
                        <td style={{ fontSize: 12 }} colSpan={4}>附表（库存款支付收）</td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td style={{ fontSize: 12 }}>合计</td>
                        <td style={{ fontSize: 12 }}>{list.filter(item => ['销售商品捆绑销售方案款', '预约办理商品捆绑销售方案'].includes(item.mode))?.reduce((a, b) => a + parseFloat(b.num), 0)}</td>
                        <td style={{ fontSize: 12 }}>{list.filter(item => ['销售商品捆绑销售方案款', '预约办理商品捆绑销售方案'].includes(item.mode))?.reduce((a, b) => a + parseFloat(b.paycash)
...
export default CollectionReportOfFinance;
