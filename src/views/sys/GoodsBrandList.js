...
const GoodsBrandList = () => {
    ...
    return (
        <form>
            <NavCard title="商品品牌列表" subtitle="系统参数设置" />
            <Card sx={{
                mt: 1,
                p: 2
                    ...
                    <Box display={"flex"} mb={3}>
                        <Button onClick={async e => {
                            const rew = await request('post', '/api/getInfo', {
                                url: 'Srapp.Web_SystemInfo.GoodsBrandList'
                            })
            ...
            <Modal visible={open} onCancel={() => setopen(false)} footer={<></>} style={{ top: '10%' }}>
                <Form getFormApi={e => api.current = e} onSubmit={async e => {
                    // console.log(e)
                    const rew = await request('post', '/api/getInfo', {
                        url: 'Srapp.Web_SystemSetting.SettingGoodsBrand',
                        ...e
                    })
                    ...
                    <Form.Input field={'name'} label={'品牌名称'} />
