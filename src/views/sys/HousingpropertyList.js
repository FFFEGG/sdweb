        ...
        <Box p={3} borderRadius={1} bgcolor={'#fff'}>
            <Box display={"flex"}>
                <Button onClick={async e => {
                    const rew = await request('post','/api/getInfo',{
                        url: 'Srapp.Web_SystemInfo.HousingpropertyList'
                    })
            ...
            <Modal visible={open} onCancel={()=>setopen(false)} footer={<></>} style={{top:100}}>
                <Form onSubmit={async e => {
                    // console.log(e)
                    const rew = await request('post','/api/getInfo',{
                        url: 'Srapp.Web_SystemSetting.SettingHousingproperty',
                        ...e
                    })
...
export default HousingpropertyList;
